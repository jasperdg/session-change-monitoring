 ...
 * @param {string} date - Date string in YYYY-MM-DD format (UTC)
 * @returns {Promise<Object>} Object with lowest and highest peaks with context
 */
async function getDailyOutliers(tableName = 'composite_rates', date) {
  const queryStart = Date.now();
  try {
    const safeTableName = validateTableName(tableName);
    
    dbLog('info', 'Fetching daily outliers', { tableName: safeTableName, date });
